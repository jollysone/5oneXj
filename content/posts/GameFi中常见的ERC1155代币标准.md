---
title: "GameFi中常见的ERC1155代币标准"
slug: solidity-erc1155
date: 2022-09-07T11:12:14+08:00
draft: false
author: "5oneXj"
...
简单来说，`ERC1155`与之前介绍的非同质化代币标准ERC721类似：在`ERC721`中，每个代币都有一个`tokenId`作为唯一标识，每个`tokenId`只对应一个代币；而在`ERC1155`中，每一种代币都有一个`id`作为唯一标识，每个`id`对应一种代币。这样，代币种类就可以非同质的在同一个合约里管理了，并且每种代币都有一个网址`uri`来存储它的元数据，类似`ERC721`的`tokenURI`。下面是`ERC1155`的元数据接口合约`IERC1155MetadataURI`：
...
### 2. 查看元数据`uri`
    ![](https://raw.githubusercontent.com/jollysone/Picture-Library/master/blog/202303032335544.png)

### 3. `mint`并查看持仓变化
`mint`一栏中输入账户地址、`id`和数量，点击`mint`按钮铸造。若数量为`1`，则为非同质化代币；若数量大于`1`，则为同质化代币。

![](https://raw.githubusercontent.com/jollysone/Picture-Library/master/blog/202303032337267.png)

`blanceOf`一栏中输入账户地址和`id`查看对应持仓

![](https://raw.githubusercontent.com/jollysone/Picture-Library/master/blog/202303032338401.png)

### 4. 批量`mint`并查看持仓变化
`mintBatch`一栏中输入要铸造的`ids`数组以及对应的数量，两者数组的长度必须相等

![](https://raw.githubusercontent.com/jollysone/Picture-Library/master/blog/202303032338335.png)

将刚刚铸造好的代币`id`数组输入即可查看

![](https://raw.githubusercontent.com/jollysone/Picture-Library/master/blog/202303032339669.png)
...
![](https://raw.githubusercontent.com/jollysone/Picture-Library/master/blog/202303032339461.png)

对刚才转入的地址查看其持仓变化。

![](https://raw.githubusercontent.com/jollysone/Picture-Library/master/blog/202303032340278.png)

## 总结

以太坊`EIP1155`提出的`ERC1155`多代币标准，它允许一个合约中包含多个同质化或非同质化代币。并且，我们创建了魔改版无聊猿 - `BAYC1155`：一个包含`10,000`种代币且元数据与`BAYC`相同的`ERC1155`代币。目前，`ERC1155`主要应用于`GameFi`中。但随着元宇宙技术不断发展，这个标准估计会越来越流行。
