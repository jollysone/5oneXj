---
title: "多签钱包要比硬件钱包更加安全"
slug: solidity-multisig-wallet
date: 2022-12-30
draft: false
author: "5oneXj"
...
V神曾说过，多签钱包要比硬件钱包更加安全（[推文](https://twitter.com/VitalikButerin/status/1558886893995134978?s=20&t=4WyoEWhwHNUtAuABEIlcRw)）。极简版多签钱包合约教学代码（150行代码）由[Gnosis Safe合约](https://gnosis-safe.io)（几千行代码）简化而成。

![](https://raw.githubusercontent.com/jollysone/Picture-Library/master/blog/202303052339697.png)
...
4. 调用多签合约的执行函数，验证签名并执行交易（链上）。对验证签名和执行交易不了解的，
...
1. 部署多签合约，`2`个多签地址，交易执行门槛设为`2`。
    ...
    ![部署多签合约](https://raw.githubusercontent.com/jollysone/Picture-Library/master/blog/202303052349116.png)

2. 转账`1 ETH`到多签合约地址。

    ![转账到多签合约地址](https://raw.githubusercontent.com/jollysone/Picture-Library/master/blog/202303052349411.png)
...
3. 调用`encodeTransactionData()`，编码并计算向多签地址1转账`1 ETH`的交易哈希。
    ...
    ![计算交易哈希](https://raw.githubusercontent.com/jollysone/Picture-Library/master/blog/202303052350041.png)
...
4. 利用Remix中ACCOUNT旁边的笔记图案的按钮进行签名，内容输入上面的交易哈希，获得签名，两个钱包都要签。
    ...
    ![多签地址签名](https://raw.githubusercontent.com/jollysone/Picture-Library/master/blog/202303052350324.png)

5. 调用`execTransaction()`函数执行交易，将第3步中的交易参数和打包签名作为参数传入。可以看到交易执行成功，`ETH`被转出多签。

    ![执行多签钱包交易](https://raw.githubusercontent.com/jollysone/Picture-Library/master/blog/202303052350194.png)
